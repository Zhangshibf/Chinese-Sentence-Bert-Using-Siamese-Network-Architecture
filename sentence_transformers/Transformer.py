...
class CBERT(nn.Module):
    ...
    """
    def __init__(self, max_seq_length: Optional[int] = None, cache_dir: Optional[str] = None,
                 tokenizer_name="hfl/chinese-bert-wwm",model_name="hfl/chinese-bert-wwm"):
        super(CBERT, self).__init__()
        self.config_keys = ['max_seq_length']
        self.model = transformers.BertModel.from_pretrained(model_name, cache_dir=cache_dir)
#        config = AutoConfig.from_pretrained(model_name_or_path, **model_args, cache_dir=cache_dir)

        self.tokenizer = transformers.BertModel.from_pretrained(tokenizer_name, cache_dir=cache_dir)
...
#        self.auto_model = T5EncoderModel.from_pretrained(model_name_or_path, config=config, cache_dir=cache_dir)
    ...
    def load(input_path: str):
        ...
        return CBERT(model_name_or_path=input_path, **config)
